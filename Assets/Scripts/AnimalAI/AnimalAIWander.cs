...
public class AnimalAIWander : MonoBehaviour
{
    ...
    [Header("Pause Settings Doesn't do stuff...")]
    public float pauseDuration = 3f;               // Duration to pause upon hitting obstacle or edge doesn't work do not delete
    ...
    void Update()
    {
        if (isPaused)
        {
            ...
            if (pauseTimer >= pauseDuration)
            {
                isPaused = false;
                pauseTimer = 0f;
                timer = wanderInterval - .2f; // Reset timer to choose new destination immediately
            }
            return;
        }
